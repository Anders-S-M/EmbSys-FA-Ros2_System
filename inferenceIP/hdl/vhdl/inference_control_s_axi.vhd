...
entity inference_control_s_axi is
generic (
...
port (
    ...
    in_r_address0         :in   STD_LOGIC_VECTOR(10 downto 0);
    in_r_ce0              :in   STD_LOGIC;
    in_r_q0               :out  STD_LOGIC_VECTOR(31 downto 0)
);
end entity inference_control_s_axi;
...
architecture behave of inference_control_s_axi is
    ...
    constant ADDR_IN_R_BASE   : INTEGER := 16#2000#;
    constant ADDR_IN_R_HIGH   : INTEGER := 16#3fff#;
    ...
    -- memory signals
    signal int_in_r_address0   : UNSIGNED(10 downto 0);
    ...
    signal int_in_r_address1   : UNSIGNED(10 downto 0);
    ...
    component inference_control_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component inference_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_in_r
int_in_r : inference_control_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 1200,
     AWIDTH   => log2(1200))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_in_r_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    -- in_r
    int_in_r_address0    <= UNSIGNED(in_r_address0);
    int_in_r_ce0         <= in_r_ce0;
    int_in_r_we0         <= '0';
    int_in_r_be0         <= (others => '0');
    int_in_r_d0          <= (others => '0');
    in_r_q0              <= STD_LOGIC_VECTOR(RESIZE(int_in_r_q0, 32));
    int_in_r_address1    <= raddr(12 downto 2) when ar_hs = '1' else waddr(12 downto 2);
...
entity inference_control_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity inference_control_s_axi_ram;

architecture behave of inference_control_s_axi_ram is
